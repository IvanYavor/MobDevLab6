...
class FunctionView: UIView {
	...
	override func draw(_ rect: CGRect) {
		...
		UIColor.red.set()
		bezier = UIBezierPath()
		bezier.move(to: Zero + CGPoint(x: rect.width/2 - Unit/4, y: -4))
		bezier.addLine(to: CGPoint(x: rect.width, y: rect.height/2))
		bezier.addLine(to: Zero + CGPoint(x: rect.width/2 - Unit/4, y: 4))
		bezier.stroke()
		...
		UIColor.green.set()
		bezier = UIBezierPath()
		bezier.move(to: Zero + CGPoint(x: -4, y: -rect.height/2 + Unit/4))
		bezier.addLine(to: CGPoint(x: rect.width/2, y: 0))
		bezier.addLine(to: Zero + CGPoint(x: 4, y: -rect.height/2 + Unit/4))
		bezier.stroke()
		...
		for x in stride(from: CGFloat(-4), to: CGFloat(4), by: CGFloat(0.001)) {
			let x = CGFloat(x)
			let y = log2f(x)
